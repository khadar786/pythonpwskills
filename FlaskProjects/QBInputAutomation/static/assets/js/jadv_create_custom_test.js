//Custom Test App
var API_URL='api/JadvCustomTestService.php';
//wizard setup
var _ctwizardEl;
var _ctformEl;
var _ctwizard;
...
var service_root='jadv_compose_services/';
...
composeapp.controller('CustomTestComposerCtrl',function($scope,$http,$timeout){
	$http.defaults.headers.post["Content-Type"] = "application/x-www-form-urlencoded";
	  ...
	  $scope._initWizard=function(){
				 ...
				 if(status=='Valid'){
		            ...
		            if(wizard.getStep()==1){
			            ...
			            var params=$.param({'course_id':$scope.current_course,
	                                  ...
	                                  "selected_subjects":angular.toJson($scope.selected_subjects),
		...
		$scope.orderUp = function(key,index){
			console.log(index);
			$scope.move(key,index, index - 1); 
			console.log(index);
		}
		$scope.orderDown = function(key,index){
    	console.log(index);
			$scope.move(key,index, index + 1); 
			console.log(index);
    }
		$scope.move = function (key,origin, destination) {
				console.log($scope.subjects[key].qtypes[destination]);
        console.log($scope.subjects[key].qtypes[origin]);

        var temp = $scope.subjects[key].qtypes[destination];
        $scope.subjects[key].qtypes[destination] = $scope.subjects[key].qtypes[origin];
        $scope.subjects[key].qtypes[origin] = temp;
      ...
      $scope.getSubjects=function(course_id){
        ...
        var params = $.param({'course_id':course_id,'ct_id':$scope.ct_id,'action':'getsubjects','user_id':$scope.user_id,'level':$scope.level});
        $http({method: 'POST',url:API_URL,data: params}).success(function(response, status, headers, config) {
          $scope.subjects=response.subjects;
	...
	$scope.select_qtype_section=function(subject_index,section_index)
	{
		console.log($scope.subjects[subject_index].selected_qcnt);
		$scope.data = $scope.subjects[subject_index].qtypes[section_index];
		if($scope.data.questions_size == 0 || $scope.data.marks_per_question == 0){
			$('#qtyrow_'+section_index+'_'+subject_index).addClass('table-danger');
	         Swal.fire({
		        	title: '<strong>'+$scope.subjects[subject_index].category+' - '+$scope.data.qtype+'</strong>',
		            icon: "error",
		            html:"<b style='color:red;'>Please provide the questions and marks !</b>",
		            buttonsStyling: false,
		            confirmButtonText: "Ok, got it!",
					customClass: {
						confirmButton: "btn font-weight-bold btn-light"
	                ...
	                $timeout(function(){$('#qtyrow_'+section_index+'_'+subject_index).removeClass('table-danger');},500);
			  });
		}else{
			$scope.data.checked = true;
			$scope.data.total_marks = ($scope.data.questions_size*$scope.data.marks_per_question);
			$scope.testSize = $scope.testSize+$scope.data.questions_size;
			$scope.for_total_score = $scope.for_total_score+$scope.data.total_marks;
			$scope.subjects[subject_index].selected_qcnt = $scope.subjects[subject_index].selected_qcnt+$scope.data.questions_size;
		}
    ...
    $scope.selectSubject=function(key_index){
      ...
      /*if($scope.subjects[key_index].selected_qcnt>$scope.subjects[key_index].total_qcnt){
       ...
       //Question exceed
       if($scope.subjects[key_index].selected_qcnt>0){
         ...
         Swal.fire({
	        	title: '<strong>'+subject_name+'</strong>',
	            icon: "error",
	            html:"<b style='color:red;'>Please provide the questions !</b>",
	            buttonsStyling: false,
	            confirmButtonText: "Ok, got it!",
				customClass: {
					confirmButton: "btn font-weight-bold btn-light"
        ...
        $http({method: 'POST',url:API_URL,data: params}).success(function(response, status, headers, config) {
            ...
            if(response.update_status==true){
              ...
              $timeout(function(){
                ...
                location.replace("jee-adv-custom-test-list");
              },2000);
            }
            if ($scope.$root.$$phase != '$apply' && $scope.$root.$$phase != '$digest') {
                 $scope.$apply();
    ...
    $scope.getChapterQuestions=function(chapter_index){
      if(chapter_index!=null && chapter_index!=""){
        ...
        var params = $.param({'key_index':chapter_index,'chapter_id':$scope.chapter_id,'comman_qlist':$scope.current_subject.comman_qlist,'selected_q':$scope.all_subject_chapters[$scope.chapter_id].selected_q,'course':$scope.course_id,'subject_id':$scope.subject_id,'qchoice_type':$scope.qchoicetype,'action':'getchapter_questions','user_id':$scope.user_id,'level':$scope.level});
    ...
    $scope.getSelectedQuestions=function(key_index,chapter_id){
      ...
      //console.log($scope.allchapters[chapter_id].selected_q);
      if ($scope.$root.$$phase != '$apply' && $scope.$root.$$phase != '$digest') {
           $scope.$apply();
       }
    };
    ...
    $scope.getTotalSelectedQuestions=function(){
      ...
      //console.log($scope.total_selected_q);
    ...
    $scope.saveSelectedQuestions=function(){
      ...
      //return false;
      var params = $.param({'compose_type':$scope.compose_type,'subject_index':$scope.subject_index,'questions':$scope.allselectedQ,'mid':$scope.current_subject.mid,'test_id':$scope.current_subject.test_id,'selected_qcnt':$scope.current_subject.selected_qcnt,'action':'savequestions','user_id':$scope.user_id,'level':$scope.level});
      $http({method: 'POST',url:API_URL,data: params}).success(function(response, status, headers, config) {
          ...
          if(response.error==false){
            ...
            $scope.subjects[index].classes=response.subject[$scope.subject_index].classes;
            $scope.current_subject.chapters=response.subject[$scope.subject_index].chapters;
            $scope.current_subject.classes=response.subject[$scope.subject_index].chapters;
            //$scope.subject_chapters=$scope.current_subject.chapters;
    ...
    $scope.deleteQuestion=function(question_index){
      ...
      var params = $.param({'qindex_key':question_index,'view_subject_index':$scope.view_subject_index,'delrecord':$scope.question,'subject':JSON.stringify($scope.view_subjectq),'action':'delete_q'});
      $http({method: 'POST',url:API_URL,data: params}).success(function(response, status, headers, config) {
        if(response.error==false){
    ...
    $scope.UnassignedStudents=function(){
		...
		$scope.currentPage = 1;
		var params=$.param({course_id:$scope.course_id,test_id:$scope.ct_id});
		$http({method: 'POST',url:service_root+'getUnassignedStudents.php',data: params}).success(function(data, status, headers, config) {
	      	//console.log(data);
  ...
  $scope.AssignedStudents=function(){
		...
		var params=$.param({course_id:$scope.course_id,test_id:$scope.ct_id});
		$http({method: 'POST',url:service_root+'getAssignedStudents.php',data: params}).success(function(data, status, headers, config) {
	      	//console.log(data);
    ...
    $scope.assignToEnroll=function(param){
      ...
      var params = $.param({'course_id':$scope.current_course,'test_id':$scope.ct_id,'students':$scope.selected_students,'assignto':param});
