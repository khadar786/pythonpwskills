...
AddNotification={
		...
		$('#addbtn').on('click',function(e){
			...
			_validation.validate().then(function(status){
				...
				if(status=='Valid'){
					...
					formData.append('campus_id',addnotification.querySelector('[name="campus_id"]').value);
					...
					formData.append('action','createnotifi');
					axios({
					  method: 'post',
					  url:API_URL,
					  data: formData,
	...
	getSections:function(cls_id){
		...
		if(cls_id!=""){
			class_id=cls_id;
			jQuery.post(API_URL,{action:"sections",class_id:class_id,campus_id:campus_id})
			  .done(function( data ) {
