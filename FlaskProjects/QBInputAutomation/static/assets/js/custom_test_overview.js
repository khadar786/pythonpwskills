...
app.controller('CTOverviewCtrl',function($scope,$http,$timeout,$compile,$sce){
	...
	$scope.init=function(){
		$scope.loader=true;
		var params=$.param({"action":"user_test_info",user_id:$scope.user_id,utid:$scope.utid});
	...
	$scope.LoadStrengthsWeaknessesMock = function(){
		var params=$.param({"action":"strengths_weaknesses",user_id:$scope.user_id,utid:$scope.utid,subject_id:$scope.subject_id,test_type:$scope.test_type});
		$http({method:'POST',url:$scope.OTR_API,data:params}).success(function(data,status,headers,config){
