...
TestApp.controller('AdaptiveTestCtrl', function($scope, $http, $filter, $timeout) {
    ...
    $scope.API_URL = site_url + 'api/OlympiadPracticeService.php';
    ...
    $scope.show_topic_result_popup = false;
    $scope.current_topic_name = "";
    $scope.next_topic_name = "";
    ...
    $scope.TopicResultModal= function(){
        if($scope.show_topic_result_popup == true && $scope.user_level_status==0){
    ...
    $scope.skipQ = function() {
        ...
        if ($scope.updateTotalQuestionsStatus()) {
            ...
            $http({ method: 'POST', url: $scope.API_URL, data: params }).success(function(response, status, headers, config) {
                ...
                $scope.current_topic_name = response.current_topic_name;
                $scope.next_topic_name = response.next_topic_name;
                $scope.btn_skip = false;
                $scope.loadQuestion();
            });
        }
    ...
    $scope.submitQ = function() {
        ...
        if ($scope.updateTotalQuestionsStatus()) {
            ...
            $http({ method: 'POST', url: $scope.API_URL, data: params }).success(function(response, status, headers, config) {
                ...
                $scope.show_topic_result_popup = response.show_topic_result_popup;
                $scope.current_topic_name = response.current_topic_name;
                $scope.next_topic_name = response.next_topic_name;
                $scope.current_level = response.current_level;
                $scope.nextdisable = false;
                ...
                if($scope.show_topic_result_popup == true){
                    $scope.TopicResultModal();
                }
                /*if(response.is_level_complete==true){
