function renderMaths(elements) {
    ...
    $(".qloader_overlay").fadeOut("slow");

}
var TestApp = angular.module('AdaptiveTestApp', ['ngSanitize','angular-useragent-parser', 'angular-virtual-keyboard']);
TestApp.config(['VKI_CONFIG', function(VKI_CONFIG) {
              VKI_CONFIG.layout.Numerico = {
                'name': "Numerico", 'keys': [
                [["1", '1'], ["2", "2"], ["3", "3"],["Backspace", "Bksp"]],
...
TestApp.controller('AdaptiveTestCtrl', function($scope, $http, $filter, $timeout) {
    ...
    $scope.API_URL = site_url + 'api/AdaptiveService.php';
    ...
    $scope.loadConfig = function() {
        ...
        $http({ method: 'POST', url: $scope.API_URL, data: params }).success(function(data, status, headers, config) {
            ...
            $scope.user_level_status = data.user_levels_completed;
            $scope.adaptive_actions = data.adaptive_actions;
            $scope.is_adp_level== data.is_adp_level;
    ...
    $scope.loadQuestion = function() {
        $scope.numeric_show = true;
        $scope.qview = false;
        var params = $.param({ "action": "loadquestion", "course_id": $scope.course_id, "subject_id": $scope.subject_id, "chapter_id": $scope.chapter_id, "topic_id": $scope.topic_id, "flag": $scope.flag, "user_id": $scope.user_id });
        $http({ method: 'POST', url: $scope.API_URL, data: params }).success(function(data, status, headers, config) {
    ...
    $scope.submitQ = function() {
        ...
        if ($scope.selectedOption.option_index == $scope.dec($scope.question.answer) || $scope.question.userAnswer == $scope.dec($scope.question.answer)) {
            //optclass= 'correct';
    ...
    //Set Selected Option Numeric
    $scope.setSelectedOptionNumeric = function(event, option) {
        console.log(option);
        ...
        var optclass = "";
        $scope.selectedOption = option;
        $scope.question.userAnswer = $scope.selectedOption;
    ...
    $scope.nextQ = function() {
        if ($scope.is_level_complete == true) {
            return false;
        }
