...
KeyUpdateApp.controller('KeyUpdateCtrl',function($scope,$http,$timeout){
    ...
    $scope.fetchAllCampus=function(){
    	$scope.loader=true;
    	var params=$.param({action:'allcampusadv',
    ...
    $scope.fetchQuestionList=function(){
    	$scope.loader=true;
    	var params=$.param({action:'fetchqidsadv',
    						campus_id:userInfo.campus_id,
    						user_id:userInfo.user_id,
    						level:userInfo.level,
    						course_id:userInfo.course_id,
    ...
    $scope.applyQuestionTypes=function(){
        $scope.campusList.forEach((value,key)=>{
            let qid=value.id;
            if(value.qtypes==3){
                let options=value.options;
                options.forEach((optvalue,pkey)=>{
    ...
    $scope.updateQuestionOption=function(qkey,update_option){
        ...
        if(update_option!=""){
            //console.log($scope.qSList[qkey].qtypes);
            if($scope.qSList[qkey].qtypes==2 || $scope.qSList[qkey].qtypes==7){
                //console.log($scope.qSList[qkey]);
                let qoptions=$scope.qSList[qkey].options.filter((value)=>value.is_checked==true);
                let qanswers=[];
                qoptions.forEach((value,key)=>{
                    qanswers.push(value.option_index);
                });

                //console.log(qanswers);
                $scope.qSList[qkey]['new_answer']=qanswers.join(",");
                //console.log($scope.qSList[qkey]);
            }else{
            ...
            //console.log($scope.qSList[qkey]['new_answer']);
            ...
            if($scope.qSList[qkey].qtypes==2 || $scope.qSList[qkey].qtypes==7){
                ...
                $scope.qSList[qkey].options.forEach((value,key)=>{
                    $scope.qSList[qkey].options[key]['is_checked']=false;
                });

                if($scope.qSList[qkey].answer!=""){
                    let setAnswer=$scope.qSList[qkey].answer.split(",");
                    $scope.qSList[qkey].options.forEach((value,key)=>{
                        if(setAnswer.includes(value.option_index.toString())){
                            $scope.qSList[qkey].options[key]['is_checked']=true;
                        }
                    });
                }
            }
    ...
    $scope.updateKey=function(){
        ...
        Questions=[];
    	$scope.qSList.forEach((value,key)=>{
    		if(value.update_option=="keychange" || value.update_option=="addscore"){
        ...
        selectedQuestions.forEach((value,key)=>{
            ...
            new_answer_arr="";
            if(value.qtypes==2 || value.qtypes==7){
                new_answer_arr=value.new_answer.split(",")
            }
    ...
    $scope.updateTestScore=function(ckey){
        return new Promise((resolve,reject)=>{
            ...
            var params=$.param({action:'updateTestScoreadv',
                           ckey:ckey,
    ...
    $scope.updateTestQuestions=function(){
            var params=$.param({action:'updateTestQuestionsadv',
                                selectedQuestions:JSON.stringify(Questions)
                               });
            $http({method:'POST',url:API,data:params})
            .success(function(response, status, headers, config) {
    ...
    $scope.refreshModal=function(){
        ...
        window.location.replace("adv-key-change-"+testconfig.test_id+"-"+userInfo.course_id);
    }
