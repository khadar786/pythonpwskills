...
google.charts.load('visualization', '1.1', {'packages':['bar','corechart', 'controls']});
  ...
  //var app = angular.module('myApp', ['myApp.Controllers','ui.bootstrap','angularUtils.directives.dirPagination']);
		...
		angular.module("myApp.Controllers",['angular-loading-bar']).controller('MentorTestCtrl',function($scope,$http,$timeout,cfpLoadingBar){
			...
			$scope.LoadClassWiseData=function(){
				$scope.NoData = true;
				$scope.LoadMockTests(course_id);
			...
			$scope.LoadMockTests = function(course_id){
				$scope.NoData = true;
				$http.get("load_custom_tests_list_mentor.php?course_id="+course_id+"&subject_id="+subject_id+"&class_id="+$scope.mentor_classes+"&size="+$scope.test_list_size).then(function (response) {
					$scope.MTests = response.data;
					if($scope.MTests.length==0) 
					{ $scope.NoData = true; console.log('No Tests in Selected Course : '+course_id); $scope.loader=false;}
			...
			$scope.LoadTestScores = function(test_id){
				$http.get("load_custom_test_data_scores_mentor.php?test_id="+$scope.current_test+"&subject_id="+subject_id+"&class_id="+$scope.mentor_classes+"&section_id="+$scope.mentor_sections+"&user_id="+user_id)
				.then(function (response) {
					...
					$("#report_download_link").attr("href", "download_subject_analysis_custom_report_per_test.php?course_id="+course_id+"&test_id="+$scope.current_test+"&subject_id="+subject_id+"&class_id="+$scope.mentor_classes+"&section_id="+$scope.mentor_sections+"&user_id="+user_id);
					  }
				...
				$http.get("load_mentor_subject_custom_speed_distribution.php?course_id="+course_id+"&test_id="+$scope.current_test+"&subject_id="+subject_id+"&user_id="+user_id+"&class_id="+$scope.mentor_classes+"&section_id="+$scope.mentor_sections).then(function(response)
				{		
			...
			$scope.LoadDistribution = function(){
				...
				test_id = $scope.current_test;
				$http.get("load_mentor_subject_custom_scores_distribution.php?course_id="+course_id+"&test_id="+$scope.current_test+"&subject_id="+subject_id+"&user_id="+user_id+"&class_id="+$scope.mentor_classes+"&section_id="+$scope.mentor_sections).then(function(response)
				{		
					$scope.SubDist = response.data;	
					var Sub_chart_heads =  Object.keys($scope.SubDist);
					//console.log(Sub_chart_heads);
			...
			$scope.LoadChapters = function(){
				test_id = $scope.current_test;
				$http.get("load_test_subject_custom_wise_topics.php?subject_id="+subject_id+"&test_id="+$scope.current_test).then(function(response)
				{		
			...
			$scope.LoadChapterDistribution = function(topic_id,Selected_Category){
				$http.get("load_mentor_chapter_custom_scores_distribution.php?course_id="+course_id+"&test_id="+$scope.current_test+"&topic_id="+topic_id+"&user_id="+user_id+"&class_id="+$scope.mentor_classes+"&section_id="+$scope.mentor_sections+"&subject_id="+subject_id).then(function(response)
				{		
			...
			$scope.getChaptersReport=function(chapter_id){
				...
				$scope.currentPage = 1;
				var params = $.param({"chapter_id":chapter_id,"test_id":$scope.current_test,"class_id":$scope.mentor_classes,"section":$scope.mentor_sections,"user_id":user_id,"subject_id":$scope.subject_id});
			...
			$scope.LoadChapterSpeedDistribution = function(topic_id,Selected_Category){
				$http.get("load_mentor_chapter_custom_speed_distribution.php?course_id="+course_id+"&test_id="+$scope.current_test+"&topic_id="+topic_id+"&user_id="+user_id+"&class_id="+$scope.mentor_classes+"&section_id="+$scope.mentor_sections+"&subject_id="+subject_id).then(function(response)
				{		
			...
			$scope.loadMockTests_Heatmap=function(page,pagesize){
				...
				$http.get("load_custm_tests_heatmap_mentor.php?test_id="+$scope.current_test+"&subject_id="+subject_id+"&user_id="+user_id+"&class_id="+$scope.mentor_classes+"&section_id="+$scope.mentor_sections+"&start="+start+"&pagesize="+pagesize+"&course_id="+course_id)
				.then(function (response) {
			...
			$scope.loadMockTests_HeatmapNew=function(pageNumber){
				$http.get("load_custm_tests_heatmap_mentor.php?test_id="+$scope.current_test+"&subject_id="+subject_id+"&user_id="+user_id+"&class_id="+$scope.mentor_classes+"&section_id="+$scope.mentor_sections+"&course_id="+course_id+'&page='+pageNumber+'&pagesize='+$scope.mt_limit)
				.then(function (response) {
					$scope.MTestsHeatmap = response.data.Students;
					$scope.Start = parseInt(response.data.Start);
			...
			$scope.LoadStrengthsWeaknessesMock = function(){
				$http.get("load_strengths_weaknesses_on_custom_test_mentor.php?test_type=MT&subject_id="+subject_id+"&user_id="+user_id+"&test_id="+$scope.current_test+"&class_id="+$scope.mentor_classes+"&section_id="+$scope.mentor_sections)
				.then(function (response) {
			...
			$scope.loadCourseSections=function(){
				$http.get("load_course_sections_mentor.php?course_id="+course_id+"&user_id="+user_id+"&class_id="+$scope.mentor_classes)
				.then(function (response) {
