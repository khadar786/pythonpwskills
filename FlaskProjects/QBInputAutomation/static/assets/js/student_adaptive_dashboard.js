...
Dashboard.controller('SubjectDashboardCtrl', function($scope, $http, $timeout) {
    ...
    $scope.getAdaptiveSubjectwiseData = function() {
        var params = $.param({ "action": "adaptive_subjectwise_data", "userid": $scope.user_id, "subject_id": $scope.subject_id, "course_id": $scope.course_id, "class_id": $scope.class_id });
    ...
    $scope.getStudentAdaptiveData = function() {
        var params = $.param({ "action": "student_adaptive_data", "userid": $scope.user_id, "subject_id": $scope.subject_id, "course_id": $scope.course_id, "class_id": $scope.class_id });
        $http({ method: 'POST', url: API_URL, data: params }).success(function(data, status, headers, config) {
