...
function show_internetconnection_pop(result){
    //console.log(result);
    if(result==false){
        $('#internetconnection').modal({
          keyboard: false
        });
    }else{
        $('#internetconnection').modal("hide");
    }
}
...
const checkOnlineStatus = async () => {
  try {
    const online = await fetch(window.location.protocol+"//"+window.location.host+"/check_online_temp.php");
...
/*  const statusDisplay = document.getElementById("internet_status");
  statusDisplay.textContent = result ? "Online" : "Offline";*/
//   show_internetconnection_pop(result);
}, 3000); // probably too often, try 30000 for every 30 seconds

// Internet connection check end