...
AddEvent={
			    ...
			    if(subjects.length>0){
			    	...
			    	for(var i=0;i<subjects.length;i++){
			    		...
			    		if(is_sub_selected==false || is_cls_selected==false || sec_cnt==0){
			    			...
			    			if(is_sub_selected && is_cls_selected && sec_cnt>0){
			    				valid_data.push({
				    						  	is_sub_selected:is_sub_selected,
				    						  	is_cls_selected:is_cls_selected,
				    						  	sec_cnt:sec_cnt
				    ...
				    if(cnt==subject_valid.length){
				    	if(subject_valid.length>0 && valid_data.length==0){
						...
						axios({
							  ...
							  headers: {
								...
								if(response.data.cid>0){
									...
									_validation_w.resetForm('reset',true);
									setTimeout(function(){
										window.location.replace('add-event');
									},1000);
	...
	getTeacherSubjects:function(tid){
		...
		if(tid!=''){
			...
			$('#pqloader').show();
			  jQuery.post(API_URL,{action:"teachersubjects",teacher_id:teacher_id})
			  .done(function( data ) {
