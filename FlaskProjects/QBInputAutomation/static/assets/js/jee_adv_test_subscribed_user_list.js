...
var KTUserList = function () {
		...
		usertable=$('#kt_datatable').KTDatatable({
			...
			data: {
				...
				source: {
					read: {
						url:API_URL,
						params:{action:'jee_adv_testsubscribedlist',
	...
	return {
		...
		loginTestData:function(user_id,user_name,email,test_id){
			...
			var params={
				  	action:"jee_adv_login_test_activity",
				  	user_id:user_id,
				  	test_id:test_id
			};
			...
			.done(function( data ) {
				...
				var qstatus = ["","Not Visited","Not Answered","Answered & Saved","Marked for Review","Answered &amp; Marked for Review"];
				...
				if(utd.length == 0){
					...
					for (var i = 0; i < utd.length; i++) {
						...
						var question_status="";
						if(utd[i].is_attempted>0 && utd[i].user_answer!="" && utd[i].question_status==2){
							question_status=qstatus[3];
						}else if(utd[i].is_attempted>0 && utd[i].user_answer!="" && utd[i].question_status==4){
							question_status=qstatus[5];
						}else{
							question_status=qstatus[utd[i].question_status];
						}

						jQuery('#test_data_activity').append('<tr><td>'+(i+1)+'</td><td>'+utd[i].user_score+'</td><td>'+utd[i].user_answer+'</td><td>'+attempted[utd[i].is_attempted]+'</td><td>'+question_status+'</td><td>'+utd[i].qtype+'</td></tr>');
					};
				}
		...
		downloadReport:function(){
			...
			window.location.replace("jee_adv_download-user-login-test-activity-"+user_id+"-"+test_id);
		}
	};
}();
