...
var KTAddUser = function () {
	...
	var _initWizard = function () {
	    ...
	    $('#student_submit').on('click', function (e) {
			...
			formData.append('user_id',user_edit);
	      ...
	      xhr.open("POST", "update_teacher.php");
	      xhr.send(formData);
	...
	return {
		...
		_initCourses:function(){
			var params={
					action:'teacher_subjects',
					user_id:user_edit,
					campus_id:campus_id
				  };
			axios({
				...
				if(courses.length>0){
					...
					for(var i=0;i<courses.length;i++){
						...
						for(var j=0;j<courses[i]['subjects'].length;j++){
							...
							if(courses[i]['subjects'][j]['is_selected']){
								course+='<input type="checkbox" id="sid_'+subject_id+'" name="sid_'+subject_id+'" value="'+subject_id+'" onclick="KTAddUser.selectSubject('+j+','+subject_id+');" data-cid="'+course_id+'" data-sid="'+subject_id+'" checked/><strong class="text-primary">'+courses[i]['subjects'][j]['subject']+'</strong><span></span>';
							}else{
							...
							for(var k=0;k<courses[i]['subjects'][j]['classes'].length;k++){
								...
								if(cls_details.is_selected){
									course+='<label class="checkbox checkbox-primary mb-4"><input type="checkbox" id="cls_'+subject_id+'_'+cls_details.class_id+'" name="cls_'+subject_id+'_'+cls_details.class_id+'" data-cid="'+course_id+'" data-sid="'+subject_id+'" data-cls-id="'+cls_details.class_id+'" onclick="KTAddUser.selectClass('+j+','+k+','+subject_id+','+cls_details.class_id+');" checked/><strong>Class '+courses[i]['subjects'][j]['classes'][k]['class_name']+'</strong> <span></span></label>';
								}else{
		...
		reviewDetails:function(){
			...
			$('#user_country').text($.trim($('#country option:selected').text()));
