...
var KTUpdateResource = function() {
    ...
    var _handleResourceForm = function() {
        ...
        var form=KTUtil.getById('kt_update_resources_form');
        validation = FormValidation.formValidation(
			KTUtil.getById('kt_update_resources_form'),
			{
				fields: {
					class: {
						validators: {
        ...
        $('#kt_update_resources_submit').on('click', function (e) {
            e.preventDefault();

            validation.validate().then(function(status) {
                var default_dashboard = "resource-list"; 
              ...
              var uploadFiles = document.getElementById('upload_file').files[0];
                    ...
                    form_data.append('resource_id', form.querySelector('[name="resource_id"]').value);
...
function getTopics(chapter_id) {
            ...
            function getchapter(class_id,subject_id)
            {
                $("#getclass_id").val(class_id);
                 $.ajax({
                    url:'ajax.php',
                    type:'POST',
                    data: {action: "getchapterlist",class_id: class_id},
                    dataType: 'json',
                    success: function( json ) {
                        var options = '';
                        if(json!=null){
