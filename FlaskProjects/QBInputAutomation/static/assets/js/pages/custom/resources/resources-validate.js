        ...
        $('#kt_resources_submit').on('click', function (e) {
            ...
            validation.validate().then(function(status) {
              ...
              var uploadFiles = document.getElementById('upload_file').files[0];
                    ...
                    form_data.append('campus_id', form.querySelector('[name="campus_id"]').value);
...
jQuery(document).ready(function() {
    ...
    function getClasses(subject_id)
    {
        $("select#loadchapters").html('<option value="">Select Chapter</option>');
        $("select#loadtopics").html('<option value="">Select Topic</option>');

         $.ajax({
            url:'ajax.php',
            type:'POST',
            data: {action: "getClassesdata",subject_id:subject_id},
            dataType: 'json',
            success: function( json ) {
                var options = '<option value="">Select Class</option>';
                if(json!=null){
                for (var i = 0; i < json.length; i++) {
                    options += '<option value="' + json[i].class_id + '">' + json[i].class_name + '</option>';
                  }
                  $("select#classid").html(options);
                }
            }
        });
    }
